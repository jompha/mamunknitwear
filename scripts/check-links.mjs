...
function checkReference(ref, originFile) {
  ...
  // prefix when one is configured (e.g. /mamunknitwear/about-us/), which must
  // be stripped before mapping onto the built filesystem (dist/about-us/).
  const withoutBase =
...
for (const file of htmlFiles) {
  ...
  // when one is configured, so no manual base handling is required.
  void baseHref;
